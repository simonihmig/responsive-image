...
import { Meta, Image, ImageMeta, ImageType } from '../types';
import ResponsiveImageService from './responsive-image';
...
export default class ResponsiveImageLocalService extends Service {
  ...
  getMeta(imageName: string): ImageMeta {
    imageName = this.normalizeImageName(imageName);
    const meta = this.meta.images[imageName];
    assert(`There is no data for image ${imageName}`, meta);
